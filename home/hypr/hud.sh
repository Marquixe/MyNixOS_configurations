...
#tmux split-window -t $SESSION:0.2 -h "gping google.com 1.1.1.1 8.8.8.8"
#tmux split-window -t $SESSION:0.2 -v -l 45 "$PICK"

tmux new-session -d -s $SESSION -x 220 -y 98 "tty-clock -s -c -C 6 -f ''"
#tmux split-window -t $SESSION:0.0 -v -l 80 "bash ~/.config/hypr/hud-date.sh"
#tmux split-window -t $SESSION:0.1 -h ""
#tmux split-window -t $SESSION:0.0 -v -l 65 "bash ~/.config/hypr/hud-stats.sh"
#tmux split-window -t $SESSION:0.2 -v -l 45 "$PICK"
#tmux split-window -t $SESSION:0.2 -h "gping google.com"
